...
import {
  carRentalCategoryPath,
  carRentalCityPath,
  isCarCategorySegment,
} from '@/lib/car-rental-paths';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (isCarCategorySegment(segment)) {
    const data = await getLocationCategoryPage(city, segment);
    ...
    return pageMetadata({
      title,
      description,
      path: carRentalCategoryPath(city, segment),
  ...
  return pageMetadata({
    title: `${brandData.brand.name} Car Rental in ${brandData.location.name} | OnnRide`,
    description: `Rent ${brandData.brand.name} cars in ${brandData.location.name}. Compare verified vendors on OnnRide.`,
    path: `/car-rental/${city}/${segment}`,
    noindex: !brandData.indexable,
  });
}

export default async function CarSegmentPage({ params }: Props) {
  const { city, segment } = await params;
  ...
  if (isCarCategorySegment(segment)) {
    const [data, locations] = await Promise.all([
      getLocationCategoryPage(city, segment),
      getLocations(),
    ]);
    if (!data) notFound();
    ...
    const searchHref = `/search?locationId=${data.location.id}&vehicleClass=four_wheeler&category=${segment}&${rentalQueryString({
        ...
        <div className="section-container py-12">
          ...
          <p className="mt-4 max-w-2xl text-slate-600">
            {data.pageMeta?.introOverride ??
              `Browse ${data.category.name.toLowerCase()} from verified vendors in ${data.location.name}. Daily rates shown live — valid LMV license required.`}
          </p>
  ...
  if (!brandData.indexable) {
    redirect(carRentalCityPath(city));
  }

  const searchHref = `/search?locationId=${brandData.location.id}&vehicleClass=four_wheeler&brandId=${brandData.brand.id}`;

  return (
    <div className="section-container py-12">
      <h1 className="font-display text-3xl font-bold text-surface-900">
        {brandData.brand.name} car rental in {brandData.location.name}
      </h1>
      <p className="mt-4 text-slate-600">
        {brandData.vehicleCount} {brandData.brand.name} cars from verified vendors.
      </p>
      <Link
        href={searchHref}
        className="mt-8 inline-flex rounded-xl bg-brand-500 px-6 py-3 text-sm font-semibold text-white hover:bg-brand-600"
